...
export async function setEthReviewData() {
	...
	try {
		...
		selectedIndices.forEach((index) => {
			...
			// Match by contract or slug for robustness
			const item = data.find((item) => {
				return (
					(item.collection && item.collection.contract === nft.collection) ||
					(item.collection && item.collection.collection === nft.collection)
				);
			});
			...
			if (item) {
				...
				if (!collections.some((c) => c.contract === item.collection.contract)) {
					collections.push(item.collection); // Store the whole collection object
				}

				if (!artists.some((a) => a.address === item.artist.address)) {
					artists.push(item.artist); // Store the whole artist object
				}

				// Assign the full collection object (with contract) to the NFT
...
export function handleCollectionSave(editedCollection, index) {
	reviewData.update((data) => {
		...
		// Merge edited fields with original to preserve contract
