...
describe('utils.ts', () => {
	...
	describe('placeholderAvatar', () => {
		it('should generate correct URL for single name', () => {
			expect(placeholderAvatar('Test')).toBe(
				'https://avatar.iran.liara.run/username?username=Test&length=1'
			);
		});

		it('should generate correct URL for name with spaces', () => {
			expect(placeholderAvatar('Test User')).toBe(
				'https://avatar.iran.liara.run/username?username=TestUser&length=1'
			);
		});
