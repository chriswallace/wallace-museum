<script lang="ts">
	...
	$: processedArtworks = data.artist?.artworks ? processArtworksForEntangled(data.artist.artworks) : [];
							...
							{#each processedArtworks as item, itemIndex}
												...
												<div class="chain-preview ethereum">
													<LazyArtwork
														artwork={{
															...
															imageUrl: item.ethereum.thumbnail_url || item.ethereum.image_url,
												...
												<div class="chain-preview tezos">
													<LazyArtwork
														artwork={{
															...
															imageUrl: item.tezos.thumbnail_url || item.tezos.image_url,
								...
								{:else}
									...
									<LazyArtwork
										...
										aspectRatio="square"
										onClick={() => goto(`/artist/${data.artist.id}/${item.id}`)}
										quality={75}
										fit="cover"
										sizes="(max-width: 768px) 100vw, (max-width: 1024px) 50vw, 33vw"
										responsiveSizes={[300, 400, 500]}
...
<style lang="scss">
	...
	.artworks-grid {
		@apply grid gap-4 md:gap-6 grid-cols-1 md:grid-cols-2 lg:grid-cols-3;
	}
	...
	.chain-label {
		@apply absolute bottom-2 left-2 px-2 py-1 bg-black/70 text-white text-xs font-mono rounded-sm backdrop-blur-sm;
	}
