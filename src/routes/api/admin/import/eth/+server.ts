...
export const POST = async ({ request }) => {
	try {
		...
		if (!Array.isArray(nfts) || nfts.length === 0) {
			return new Response(JSON.stringify({ error: 'No NFTs provided for import.' }), {
				status: 400
			});
		}
				...
				try {
					...
					for (const nft of nfts) {
						...
						try {
							...
							if (mediaData.dimensions && mediaData.dimensions.width && mediaData.dimensions.height)
								normalizedMetadata.dimensions = mediaData.dimensions;
							...
							const existingArtwork = await prisma.artwork.findFirst({
								where: {
									AND: [{ tokenID: identifier }, { contractAddr: contractAddr }]
								}
							});
								...
								await prisma.artwork.update({
									...
									data: {
										...
										dimensions: normalizedMetadata.dimensions
											? JSON.stringify(normalizedMetadata.dimensions)
											: undefined,
										...
										tokenID: identifier,
										mintDate: normalizedMetadata.mintDate
											? new Date(normalizedMetadata.mintDate)
											: null,
